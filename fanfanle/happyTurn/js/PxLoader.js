...
(function(global) {
     ...
     */
    function PxLoader(settings,assets) {
...
for(var i=0 ; i<loaderAssets.length ; i++ ){
    loader.addImage(loaderAssets[i]);
}

loader.addProgressListener(function (e) {
    var number = Math.ceil(e.completedCount / e.totalCount * 100);
    var loadText=document.querySelector('#load p');
    loadText.innerHTML=number + '%';
});
...
var pxTimer = setInterval(function(){
	...
	var loadText=document.querySelector('#load p');
	if(pxTime <9 ){		
    	loadText.innerHTML=pxTime*10+parseInt(Math.random()*10) + '%';
	}else{
